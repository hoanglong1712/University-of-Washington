...
def rotate_image(input_image, theta):
    ...
    for i in range(input_rows):
      for j in range(input_cols):
        ...
        oi, oj = rotate2d(np.array([i - input_rows / 2,j - input_cols / 2]),theta)
        # 3 move image back to the old position
        oi += input_rows / 2
        oj += input_cols / 2
        if  0 <=  oi < input_rows and 0<= oj < input_cols:
          output_image[int(i), int(j) , :] = input_image[int(oi), int(oj), :]
          pass
